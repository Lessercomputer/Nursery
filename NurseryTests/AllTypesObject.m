...
- (id)init
{
    if (self = [super init])
    {
        ...
        pointValue = CGPointMake(1, 1);
        sizeValue = CGSizeMake(2, 2);
        rectValue = CGRectMake(1, 1, 2, 2);
...
- (BOOL)isEqual:(id)object
{
    ...
    if (!CGPointEqualToPoint([self pointValue], [anObject pointValue])) return NO;
    if (!CGSizeEqualToSize([self sizeValue], [anObject sizeValue])) return NO;
    if (!CGRectEqualToRect([self rectValue], [anObject rectValue])) return NO;
